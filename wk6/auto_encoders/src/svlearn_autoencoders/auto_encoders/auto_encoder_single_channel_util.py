...
from svlearn_autoencoders.auto_encoders.auto_encoder_multi_channel_util import variational_ae_loss_function
from svlearn_autoencoders.auto_encoders.variational_auto_encoder_mnist import VariationalAutoencoderMnist
from svlearn_autoencoders.auto_encoders.vqvae_auto_encoder_mnist import VqvaeAutoencoderMnist
from svlearn_autoencoders.auto_encoders.vqvae_embedding import vqvae_loss
...
def train_autoencoder(model: nn.Module, 
    ...
    for epoch in range(num_epochs):
        ...
        for images in train_loader:  # Labels are not needed
            ...
            if mode == 'contractive':
                c_factor = 1e-4
                contraction_loss = get_contractive_loss_term(hidden, images)
...
def get_contractive_loss_term(hidden: torch.Tensor, images: torch.Tensor) -> float:
    """Computes the contractive loss term

    Args:
        hidden (torch.Tensor): The hidden representation
        images (torch.Tensor): The input images

    Returns:
        float: The contractive loss term
    """
    batch_size = images.size(0)
    loss = 0.0
    for i in range(hidden.size(1)):  # loop over latent units
        grad = torch.autograd.grad(
            hidden[:, i].sum(), images,
            create_graph=True, retain_graph=True, only_inputs=True
        )[0]  
        loss += (grad ** 2).sum().item()    
