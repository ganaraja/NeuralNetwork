...
from svlearn_gan import config, current_task
from svlearn_gan.dcgan_models import TreeDiscriminator, TreeGenerator, DigitGenerator, DigitDiscriminator, load_task_specific_networks

#  -------------------------------------------------------------------------------------------------
# GENERATOR

class ConditionalGenerator(nn.Module):
    def __init__(self, generator: DigitGenerator | TreeGenerator , class_num=2):
...
class ConditionalDiscriminator(nn.Module):
    def __init__(self, discriminator: TreeDiscriminator | DigitDiscriminator, class_num: int=2):
...
def load_task_specific_conditional_networks(device, nc, nz, ngf, ndf, num_classes):
    # number of classes of training images
    num_classes = config[current_task]['num_classes']

    generator, discriminator = load_task_specific_networks(device, nc, nz, ngf, ndf)
    
    netG = ConditionalGenerator(generator, num_classes).to(device)
    netD = ConditionalDiscriminator(discriminator, num_classes).to(device)
