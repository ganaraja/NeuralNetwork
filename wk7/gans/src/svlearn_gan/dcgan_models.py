...
'''
from svlearn_gan import Task, current_task
...
class DigitDiscriminator(nn.Module):
    def __init__(self, ndf, nc):  # Adjusted ndf and nc as per typical settings
...
def load_task_specific_networks(device, nc, nz, ngf, ndf):

    if current_task == Task.TREE.value:
        generator = TreeGenerator(nz, ngf, nc )
        discriminator = TreeDiscriminator(ndf, nc)
    elif current_task == Task.MNIST.value:
        generator = DigitGenerator(nz, ngf, nc )
        discriminator = DigitDiscriminator(ndf, nc)
    else:
