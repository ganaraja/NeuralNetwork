...
class UnivariateApproximator:
    def __init__(self, func: Callable[[float], float], start: float = 0, end: float = 1, scale: bool = True, batch_size: int = 1) -> None:
        ...
        self.train_dataloader = DataLoader(TensorDataset(x_train, y_train), batch_size=batch_size)
        self.test_dataloader = DataLoader(TensorDataset(x_test, y_test), batch_size=batch_size)
    ...
    def train(self, epochs: int = 10) -> None:
        ...
        for epoch in range(epochs):
            ...
            for i, (inputs, responses) in enumerate(self.train_dataloader, 0):
                ...
                outputs = self.network(inputs.unsqueeze(1))
    ...
    def evaluate_model(self) -> List[UnivariatePrediction]:
        ...
        with torch.no_grad():  # since we are in inference-model...
            for inputs, responses in self.test_dataloader:
                batch_yhats = self.network(inputs.unsqueeze(1))
